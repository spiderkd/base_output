...
// registry/new-york/ui/checkbox.tsx — Risograph Checkbox
//
// Visual system:
...
interface CheckboxProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, "type">, RisoThemeProps {
  label?: string;
  description?: string;
}

export const Checkbox = React.forwardRef<HTMLInputElement, CheckboxProps>(
  ({ label, description, className, onChange, checked, defaultChecked, theme, primary, secondary, overlap, paper, style, ...props }, ref) => {
    ...
    return (
      <label className={cn("inline-flex items-start gap-2.5 cursor-pointer select-none group", className)} style={{ ...risoStyle, ...style }}>
        <div className="relative flex-shrink-0 mt-px w-[21px] h-[21px]">
          ...
          <svg
            ...
            width={BOX}
            height={BOX}
            className="absolute group-hover:[transform:translate(-0.5px,-0.5px)] transition-transform duration-150 top-0 left-0" filter="url(#riso-grain-border)"
          >
            <rect
              x={1} y={1}
        ...
        {(label || description) && (
          <div>
            {label && (
              <span
                className="block text-sm font-bold uppercase tracking-wide font-[family-name:var(--font-riso-label,'Space_Grotesk',sans-serif)] text-[var(--riso-overlap,#7b4f7a)]"
              >
                {label}
              </span>
            )}
            {description && (
              <span
                className="block text-[11px] mt-0.5 font-[family-name:var(--font-riso-body,'Work_Sans',sans-serif)] text-[var(--riso-secondary)]"
              >
                {description}
              </span>
            )}
