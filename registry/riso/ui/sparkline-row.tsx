...
// registry/riso/ui/sparkline-row.tsx — Risograph Sparkline Row
...
export function SparklineRow({
  ...
  const color =
    ink === "primary" ? "var(--riso-primary)" : "var(--riso-secondary)";
  const shadow =
    ink === "primary" ? "var(--riso-secondary)" : "var(--riso-primary)";
  ...
  return (
    ...
    >
      ...
      <polyline
        points={data
          .map((v, i) => {
            ...
            const y =
              height - PAD - ((v - min) / range) * (height - PAD * 2) + 1.5;
            return `${x},${y}`;
          })
          .join(" ")}
        fill="none"
      ...
      {(() => {
        ...
        const ly =
          height - PAD - ((last - min) / range) * (height - PAD * 2) - 3;
